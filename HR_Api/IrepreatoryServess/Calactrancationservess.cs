    ...
    public class SalaryclackServesses
    {
        ...
        public void CalculateSalary(string employeeId, decimal amount, DateTime transactionDateTime)
        {
            ...
            if (currentMonthInHistoryTable != firstDayOfMonth)
            {
                ...
                if (historyTransaction != null && historyTransaction.transactionTyp ==  SystemEnums.TransactionSalaryType.Bonus)
                {
            ...
            else
            {
                ...
                if (historyTransaction != null && historyTransaction.transactionTyp == SystemEnums.TransactionSalaryType.Bonus && transactionDateTime >= firstDayOfMonth && transactionDateTime <= firstDayOfMonth.AddMonths(1).AddDays(-1))
                {
                    // Subtract the transaction amount from the total salary
                    updatedHistory.TotalSalary += amount;
                }
                else if (historyTransaction != null && (historyTransaction.transactionTyp == SystemEnums.TransactionSalaryType.debt || historyTransaction.transactionTyp == SystemEnums.TransactionSalaryType.Deduction) && historyTransaction.TransactionDate >= firstDayOfMonth && historyTransaction.TransactionDate <= firstDayOfMonth.AddMonths(1).AddDays(-1))
                {
                    // Subtract the transaction amount from the total salary
                    updatedHistory.TotalSalary -= amount;
                }
