...
namespace HR_Api.Controllers
{
    ...
    public class DepartmentController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public IActionResult UpdateDepartmint(int id, [FromForm] DepartmintDTO updatedProductDto)
        {
            ...
            try
            {
                var existingProduct = _unitofwork.Deparment.Update(updatedProductDto);
        ...
        [HttpPost]
        public IActionResult CreateDepartmint([FromForm] DepartmintDTOAdd vacationDTO)
        {
            if (!ModelState.IsValid)
            {
                // If ModelState is not valid, return a BadRequest response with the validation errors
            ...
            try
            {
                ...
                var existingVacation = _unitofwork.Deparment.Add(vacationDTO);
                return CreatedAtAction(nameof(GetDepartment), new { id = existingVacation.Id }, existingVacation);
            }
            catch (Exception ex)
            {
